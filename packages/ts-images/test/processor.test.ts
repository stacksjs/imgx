...
describe('processor', () => {
  ...
  describe('optimizeImage', () => {
    it('should optimize an image', async () => {
      ...
      // Pure TS encoders may not always produce smaller files than highly optimized inputs
  ...
  describe('convertImageFormat', () => {
    it('should convert an image to webp format', async () => {
      ...
      // Verify the format
      const metadata = await readMetadata(result.outputPath)
  ...
  describe('batchProcessImages', () => {
    it('should batch process images in a directory', async () => {
      ...
      for (const filename of testImages) {
        ...
        await createSolidImage(filepath, 100, 100, { r: 255, g: 0, b: 0 })
      }
      ...
      // Note: Pure TS WebP encoder uses lossless, which may produce larger files
      // savedPercentage can be negative when converting to WebP lossless
      expect(typeof result.summary.savedPercentage).toBe('number')
    ...
    it('should filter images based on the filter option', async () => {
      ...
      for (const filename of testFiles) {
        ...
        if (filename.endsWith('.png') || filename.endsWith('.jpg')) {
          ...
          await createSolidImage(filepath, 50, 50, { r: 0, g: 255, b: 0 })
        }
        else {
    ...
    it('should use custom filename templates', async () => {
      ...
      await createSolidImage(filepath, 200, 150, { r: 255, g: 200, b: 0 })
    ...
    it('should apply transformations to images', async () => {
      ...
      await createSolidImage(filepath, 100, 100, { r: 255, g: 255, b: 255 })
    ...
    it('should track progress with a callback', async () => {
      ...
      for (const filename of testImages) {
        const filepath = join(OUTPUT_DIR, 'batch-test', filename)
        await createSolidImage(filepath, 50, 50, { r: 100, g: 150, b: 200 })
      }
